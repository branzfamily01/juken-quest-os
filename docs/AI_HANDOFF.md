...
- 本体:`index.html` **1ファイルだけ**

## 2. 絶対に守る技術的前提(壊すと動かなくなる/データが消える)

| # | 不変条件 | 理由 |
|---|---|---|
| 1 | **HTML/CSS/JS 1ファイル構成を維持**(外部ライブラリ・ビルド禁止) | ダブルクリックで動く・オフラインで動くことが製品価値 |
...
| 【21】doImport / downloadBackup | **教材JSON登録・バックアップ** | ★JSON受け入れ形式の変更 |
| 【22】起動処理 | ページを開いた直後の分岐 | |
...
5. 統合仕様書の「出題形式スキーマ」章に、JSONの書き方を追記する
6. 教材変換プロンプト(PROMPTS_FOR_NEXT_AI.md ①)にも新形式の説明を追記する
